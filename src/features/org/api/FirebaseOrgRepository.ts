...
import { OrgMembership, OrgMember } from '../types';
...
export const FirebaseOrgRepository = {
  ...
  async getMyOrgs(userId: string): Promise<OrgMembership[]> {
    ...
    try {
      ...
      return snap.docs.map(doc => ({ id: doc.id, ...doc.data() } as unknown as OrgMembership));
    } catch (error) {
  ...
  subscribeMembers(orgId: string, callback: (members: OrgMember[]) => void): Unsubscribe | null {
    ...
    return onSnapshot(membersRef, (snap) => {
      const members = snap.docs.map(doc => ({ id: doc.id, ...doc.data() } as unknown as OrgMember));
      callback(members);
    }, (error) => {
