...
export const useOrg = (notice: (message: string) => void) => {
  const orgJoin = async (orgId: string, user: User, userProfile?: UserProfile) => {
    try {
  ...
  const orgCreate = async (name: string, user: User, userProfile?: UserProfile) => {
    try {
