...
export const useProductData = (
  ...
  const exportProductsToExcel = (products: Product[]) => {
    ...
    const excelData = products.map(p => ({
      ...
      "최근실사일": p.lastAudit instanceof Date ? p.lastAudit.toLocaleDateString() : "기록없음"
    }));
    downloadFile(excelData, `${currentOrg.name}_재고현황`);
  };

  // [4] 전체 로그 엑셀 다운로드
  const getLogExcelData = async () => {
    try {
      ...
      const logExcelData = allLogs.map(d => ({
        "일시": d.timestamp instanceof Date ? d.timestamp.toLocaleString() : "N/A",
        "품목명": d.productName,
  ...
  return { exportProductsToExcel, getLogExcelData };
};